function clustering_Hybrid_3Level
...
result=[];
for dataset_no=5:5 %length(files_name)
    ...
    c=[];
    k1=k %round(rows/15)
    [c,itr]= do_kModes_time(nor_traj,k1,'Euclid',0,'SAX','alphabet_size',8,'compression_ratio',6);
    %------------------
    % Evaluation
...
%        center{i}=nor_traj{t(1,1)};
    ...
    %-----------
  %  Plot_time_series_luminate(0,0,c(:,4),p,center,nor_traj,t_traj,l2_clusterCount,2,0.2,2);

function test
% to create the clusters based on ED
d = load('LSF5_10.txt');
tL = int64(50000);
mL = int64(50);
wL = int64(500);
...
if (bsf_ed==inf)
    ...
    MDL_OLD = MDL_Cluster_i(TS,Cluster(bsf_cid))/M;   %DLC(C)
...
for j=1:size(clusi.elm,1) %%%% Careful Here %%%
    ...
    ts1 = DNorm_Unif(TS(pos:pos+M-1));
    ts1 = [zeros(1,Xshf), ts1, zeros(1,cenM-M-Xshf)];
    
    for  i=1:size(clusi.elm,1)
        if i~=j
                ...
                ts2 = DNorm_Unif(TS(pos:pos+M-1));
                ts2 = [zeros(1,Xshf), ts2, zeros(1,cenM-M-Xshf)];
            cost=cost+ED(ts1,ts2);
        end
       
    end
end
 cost=cost/( size(clusi.elm,1)*(size(clusi.elm,1)-1));

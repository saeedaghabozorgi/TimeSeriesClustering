...
while i<3000
    ...
    [Cost_Create(k)]= CostIfCreate(d,Mark,Cluster,Motif(k,:));  
    ...
    drawnow
    xlabel(h1,sprintf('M: %5.2f',Cost_Add(clusloc,1)));
    i=i+5;
end
...
function [cost_Add] = FindAddedClusterML(TS,Mark,Cluster,ci,MM,pos)     
    ...
    bsf_ed = inf;
      ...
      ed = ED(cenTS,zNorm(TS(pos:pos+M-1)));
      if (bsf_ed > ed)
            bsf_ed  = ed;                
            bsf_pos = pos;
            bsf_cid = ci; 
    ...
    if (bsf_ed==inf)
       cost_Add = [inf 0 0 0];
        ...
        MDL_OLD2 = MDL(zNorm(TS(bsf_pos:bsf_pos+M-1)))/M;   %DL(A)

...
while (step<10000) %% Fix at most 100 steps
    plot(h,(step:step+500-1),TS(step:step+500-1));
    xlim(h,[step,step+500-1]);
    ...
    for k=1:length(MM)
        ...
        if ( NoMotif(k) == true)
            ...
            if (Motif(k,1)==inf)
                ...
                t_ref = cputime;
                TTS=TS(step:step+W-1);
                MMark=Mark(step:end);
                motif = MK_Motif(TTS,M,MMark);
                Motif(k,:) = [motif, M];
                Motif(k,2)=Motif(k,2)+step;
                Motif(k,3)=Motif(k,3)+step;
                if (Motif(k,1)==inf)
                    NoMotif(k)=true;
                end
    ...
    if cost_create~=inf
        ...
        hold(h, 'on');
        plot(h,(Motif(1,2):Motif(1,2)+Motif(1,4)-1),TS(Motif(1,2):(Motif(1,2)+Motif(1,4)-1)),'r','LineWidth',2);
        plot(h,(Motif(1,3):Motif(1,3)+Motif(1,4)-1),TS(Motif(1,3):(Motif(1,3)+Motif(1,4)-1)),'r','LineWidth',2);
    else
        a=1;
     end
    ...
    for ci=1:size(Cost_Add,1)
     ...
     plot(h,(step:step+500-1),Mark(step:step+500-1)*(-500)-7000)
